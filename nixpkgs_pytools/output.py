...
def write_nixpkgs_package(content, package_name, nixpkgs_root, force=False):
    ...
    if not ({'default.nix', 'doc', 'lib', 'maintainers', 'README.md', 'nixos', 'pkgs'} <= set(os.listdir(nixpkgs_root))):
    ...
    python_modules_directory = os.path.join(nixpkgs_root, 'pkgs', 'development', 'python-modules')
    package_directory = os.path.join(python_modules_directory, normalized_package_name)
    python_packages_filename = os.path.join(nixpkgs_root, 'pkgs', 'top-level', 'python-packages.nix')
    ...
    normalized_package_names = {format_normalized_package_name(_) for _ in os.listdir(python_modules_directory)}

    # check that package does not already exist
    if normalized_package_name in normalized_package_names and not force:
        raise ValueError(f'cannot overrite existing package derivation {package_directory} without force "-f" option')
    ...
    for i, match in enumerate(re.finditer(package_regex, python_packages_content[content_offset:])):
        ...
        right_letter_index = string.ascii_lowercase.find(python_packages_package_name[0])
        letter_distance = abs(left_letter_index - right_letter_index)

        if normalized_package_name < python_packages_package_name and letter_distance <= 1:
            ...
            with open(python_packages_filename, 'w') as f:
                f.write(python_packages_content[:insertion_location] + inserted_text + python_packages_content[insertion_location:])
